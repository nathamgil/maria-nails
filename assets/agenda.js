/* =====================================================================
   Maria Nails — motor de agendamento
...
(function () {
  ...
  var CHAVE = 'mn_agendamentos_demo';
  ...
  function demoBloqueios() {
    try { return JSON.parse(localStorage.getItem('mn_bloqueios_demo') || '[]'); }
    catch (e) { return []; }
  }
  ...
  function demoSemeia() {
    try { if (localStorage.getItem('mn_demo_semeado_v2')) return; } catch (e) { return; }
    try { localStorage.removeItem(CHAVE); } catch (e) {}
    var nomes = ['Ana S.','Juliana M.','Camila A.','Beatriz C.','Larissa R.','Fernanda O.',
                 'Gabriela L.','Carla F.','Patrícia H.','Vitória B.','Amanda P.','Isabela N.'];
    ...
    demoGrava(lista);
    try { localStorage.setItem('mn_demo_semeado_v2', '1'); } catch (e) {}
  }
  ...
  // barbeiro (profissional): objeto escolhido | null (ainda não escolheu) | 'qualquer'
  var estado = { barbeiros: [], servicos: [], barbeiro: null, servico: null, dia: null, hora: null, etapa: 1 };

  // Com uma profissional só, a etapa "com quem" some e a agenda começa no serviço.
  // Entrou outra profissional no banco? A etapa volta sozinha, sem mexer no código.
  ...
  function pintaEscolhas() {
    escolhas.innerHTML = '';
    var itens = [];
    if (estado.barbeiro && !unico()) itens.push({ rot: 'Profissional', val: nomeBarbeiro(), volta: 1 });
  ...
  function pintaBarbeiros() {
    ...
    estado.barbeiros.concat(['qualquer']).forEach(function (b) {
      ...
      txt.appendChild(el('span', null, qualquer ? 'Primeira profissional livre' : (b.cargo || 'Profissional')));
      btn.appendChild(txt);
  ...
  function carregaDias(pref) {
    ...
    API.dias(estado.servico, barbeiroEscolhido()).then(function (dias) {
      ...
      if (!primeiro) {
        ...
        v.appendChild(el('p', null, 'Não há horário livre nos próximos 30 dias. Chame no WhatsApp que a gente encaixa.'));
        fita.appendChild(v);
        return;
      }
  ...
  function carregaHoras() {
    tituloDia.textContent = dataPorExtenso(estado.dia) +
      (estado.barbeiro === 'qualquer' ? ' · com a primeira profissional livre' : ' · com ' + estado.barbeiro.nome);
    caixaHoras.innerHTML = '';
  ...
  function montaResumo() {
    ...
    var dl = el('dl');
    [
      ['Profissional', estado.barbeiro === 'qualquer' ? 'O primeiro livre nesse horário' : estado.barbeiro.nome],
      ['Serviço', s.nome],
  ...
  try {
    var salvo = JSON.parse(localStorage.getItem('mn_cliente') || 'null');
  ...
  form.addEventListener('submit', function (ev) {
    ...
    if (nome.length < 2)   return erro(caixaMsg, 'Escreva seu nome para a Maria te reconhecer.');
    ...
    API.criar({
      ...
      try { localStorage.setItem('mn_cliente', JSON.stringify({ nome: nome, tel: campoTel.value })); } catch (e) {}
  ...
  function mostraSucesso(a, nome) {
    ...
    $('#ok-ics', raiz).download = 'maria-nails-' + a.codigo + '.ics';
  ...
  function fazIcs(a) {
    ...
    var linhas = [
      'BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//Maria Nails//PT-BR',
      'BEGIN:VEVENT',
      'UID:' + a.codigo + '@marianails',
      'DTSTART;TZID=America/Bahia:' + carimbo(a.dia, a.inicio),
      'DTEND;TZID=America/Bahia:'   + carimbo(a.dia, a.fim),
      'SUMMARY:' + a.servico + ' com ' + a.barbeiro + ' — Maria Nails',
  ...
  // primeiro horário livre de uma profissional para um serviço: { dia, hora } ou null
